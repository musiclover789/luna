...
var PageNavigate = func(conn *protocol.DevToolsConn, url string) gjson.Result {
...
var GetFrameTree = func(conn *protocol.DevToolsConn, id int) {
	req := map[string]interface{}{
		"id":     id,
...
var PageClose = func(conn *protocol.DevToolsConn) {
	req := map[string]interface{}{
		"id":     luna_utils.IdGen.NextID(),
		"method": "Page.close",
	}
...
func BringToFront(conn *protocol.DevToolsConn) gjson.Result {
	id := luna_utils.IdGen.NextID()
	req := map[string]interface{}{
		"id":     id,
...
func GetResourceContent(conn *protocol.DevToolsConn, frameID string, url string) {
	req := map[string]interface{}{
		"id":     luna_utils.IdGen.NextID(),
...
func GetNavigationHistory(conn *protocol.DevToolsConn) gjson.Result {
	id := luna_utils.IdGen.NextID()
	req := map[string]interface{}{
		"id":     id,
...
计算缩放因子
*/
func CalculateScalingFactorSync(conn *protocol.DevToolsConn, timeout time.Duration) (error, float64) {
	id := luna_utils.IdGen.NextID()
...
func PageCaptureScreenshotSync(conn *protocol.DevToolsConn, timeout time.Duration) ([]byte, error) {
	id := luna_utils.IdGen.NextID()
	req := map[string]interface{}{
		"id":     id,
...
func pageCaptureScreenshotRetrySync(conn *protocol.DevToolsConn, timeout time.Duration) ([]byte, error) {
	id := luna_utils.IdGen.NextID()
	req := map[string]interface{}{
		"id":     id,
		"method": "Page.captureScreenshot",
...
func CaptureTestRetry(conn *protocol.DevToolsConn, path string, timeout time.Duration) error {
	start := time.Now()
...
func CaptureTest(conn *protocol.DevToolsConn, path string, timeout time.Duration) error {
	start := time.Now()
...
func ClosePage(conn *protocol.DevToolsConn) {
	req := map[string]interface{}{
		"id":     luna_utils.IdGen.NextID(),
		"method": "Page.close",
		"params": map[string]interface{}{},
...
func ReloadPage(conn *protocol.DevToolsConn, ignoreCache bool, scriptToEvaluateOnLoad string) {
	req := map[string]interface{}{
		"id":     luna_utils.IdGen.NextID(),
		"method": "Page.reload",
		"params": map[string]interface{}{
...
func PageDisable(conn *protocol.DevToolsConn) {
	req := map[string]interface{}{
		"id":     luna_utils.IdGen.NextID(),
		"method": "Page.disable",
		"params": map[string]interface{}{},
...
func GetLayoutMetrics(conn *protocol.DevToolsConn) {
	req := map[string]interface{}{
		"id":     luna_utils.IdGen.NextID(),
