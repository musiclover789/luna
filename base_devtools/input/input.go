...
func WaitForMatchSync(conn *protocol.Session, smallImgPath, imgCachePath string, matchScore, coefficient float64, timeout time.Duration) (error, bool) {
...
*/
func GetSmallImageCoordinates(conn *protocol.Session, smallImgPath, imgCachePath string, coefficient float64, timeout time.Duration) (error, *ImageCoordinates) {
	return GetSmallImageCoordinatesWithMargin(conn, smallImgPath, imgCachePath, coefficient, 1.0, 1.0, 1.0, 1.0, timeout)
}

func GetSmallImageCoordinatesWithMarginMore(conn *protocol.Session, smallImgPaths []string, imgCachePath string, coefficient, leftMargin, rightMargin, topMargin, bottomMargin float64, timeout time.Duration) (error, *ImageCoordinates) {
	start := time.Now() // 记录开始时间
	//这个地方可能只能这样设计了,就是每秒截图一次;成功就成功，失败就继续。我们在超时之内不反回
...
func GetSmallImageCoordinatesWithMargin(conn *protocol.Session, smallImgPath, imgCachePath string, coefficient, leftMargin, rightMargin, topMargin, bottomMargin float64, timeout time.Duration) (error, *ImageCoordinates) {
	start := time.Now() // 记录开始时间
	//这个地方可能只能这样设计了,就是每秒截图一次;成功就成功，失败就继续。我们在超时之内不反回
	bigImgPath := luna_utils.ConcatenateFileName(smallImgPath, imgCachePath, "_big")
	testImgPath := luna_utils.ConcatenateFileName(smallImgPath, imgCachePath, "_test")
...
func SimulateMouseClick(conn *protocol.Session, x, y float64) {
...
func SimulateMousePressed(conn *protocol.Session, x, y float64) {
	params := mouseEventParams{
		Type:       "mousePressed",
		Button:     MouseButtonLeft,
		ClickCount: 1,
...
func SimulateMouseReleased(conn *protocol.Session, x, y float64) {
	params := mouseEventParams{
		Type:       "mouseReleased",
		Button:     MouseButtonLeft,
		ClickCount: 1,
...
func ScrollMouseToTargetImage(conn *protocol.Session, x, y float64, totalDistance int, direction Direction, smallImgPath, imgCachePath string, coefficient, matchScore float64, timeout time.Duration) (error, bool) {
...
// 滚动
func SimulateMouseScroll(conn *protocol.Session, x, y float64, totalDistance int, direction Direction) {
...
// 移动
func SimulateMoveMouse(conn *protocol.Session, startX, startY, endX, endY float64) {
...
// 键盘
func SimulateKeyboardInput(conn *protocol.Session, text string) {
	a := pinyin.NewArgs()
...
func SimulateEnterKey(conn *protocol.Session) {
	keyParamsDown := keyEventParams{
		Type:           "keyDown",
		Key:            "Enter",
		WindowsKeyCode: 13,
...
func SimulateBackspaceKey(conn *protocol.Session) {
	keyParamsDown := keyEventParams{
		Type:           "keyDown",
		Key:            "Backspace",
		WindowsKeyCode: 8,
...
func SetInterceptDrags(conn *protocol.Session) {
...
func DispatchDragEvent(conn *protocol.Session, x, y int, event string) {
	id := luna_utils.IdGen.NextID()
	// 创建拖拽事件参数
...
touch
*/
func DispatchTouchEvent(conn *protocol.Session, touchType string, x, y float64) {
	id := luna_utils.IdGen.NextID()
	req := map[string]interface{}{
		"id":     id,
